...
// first attempt
// failed test case: [3,3,3,2,5]
// dynamic programming with time complexity O(n) & space complexity is O(1)
int wiggleMaxLength(vector<int>& nums) {
	...
	int i = 1;
	int prev_dif = 0;
	while (prev_dif == 0 && i < n) {
		prev_dif = nums[i] - nums[i - 1];
		if (prev_dif != 0) break;
		i++;
	}
	int ans = prev_dif != 0 ? 2 : 1;
	for (; i < n; i++) {
		int curr_diff = nums[i] - nums[i - 1];
		if (prev_dif * curr_diff < 0) {
			ans++;
			prev_dif = curr_diff;
		}
	}
...
// TODO: understand another approach